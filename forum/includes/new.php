            ...
            */
            ...
            if ($count > 0) {
                ...
                while ($res = $stmt->fetch()) {
                    ...
                    $nick = $db->query("SELECT * FROM `forum` WHERE `refid` = '" . $res['id'] . "' AND `type` = 'm'" . ($rights >= 7 ? '' : " AND `close` != '1'") . " ORDER BY `time` DESC LIMIT 1")->fetch();
    ...
    */
    ...
    if ($stmt->rowCount()) {
        ...
        while ($res = $stmt->fetch()) {
            ...
            echo ' <span class="gray">' . functions::display_date($nam['time']) . '</span>';
            echo '</div></div>';
        }
    } else {
        echo '<div class="menu"><p>' . $lng['list_empty'] . '</p></div>';
